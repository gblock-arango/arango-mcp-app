...
but with the **arango-agent** app's SP (each deployment is separate; gateway and agent identities
...
def agent_workspace_client() -> WorkspaceClient:
    ...
    **gthread**, avoid concurrent Genie + SQL while the strip path mutates ``os.environ`` (rare).
    ...
    if not token:
        ...
        # client and Genie auto-provision never writes UC (flask-arango-agent uses plain WorkspaceClient).
        logger.info(
